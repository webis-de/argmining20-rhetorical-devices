 ...
 * 
 * This version is adapted to read files from the NYT(New York Times) corpus
 * which are in the XML format.
 * 
 **/
public class UIMAXMLReader extends CollectionReader_ImplBase {
...
//		int n = rand.nextInt(this.mFiles.size()) +1;
	    // Open input stream to file
//	    File file = (File) this.mFiles.get(n);
	    File file = (File) this.mFiles.get(this.mCurrentIndex++);
		// create parser from NYTCorpusDocumentParser
		NYTCorpusDocumentParser parser = new NYTCorpusDocumentParser();
		NYTCorpusDocument timesDocument = parser.parseNYTCorpusDocumentFromFile(file, false);
		String text = timesDocument.getBody();
		
		System.out.println("Current file: "+file.getName());
		System.out.println("Progress... "+this.mCurrentIndex+" out of "+this.mFiles.size() +" processed");
		//if (!text.isEmpty() && text.contains("LEAD:")) {
		//	text = text.substring(text.indexOf("\n")+1);
		//}
		SyntacticRhetoricSimpleAnnotator sra = new SyntacticRhetoricSimpleAnnotator();
		//get the long forms of auxiliary verbs
